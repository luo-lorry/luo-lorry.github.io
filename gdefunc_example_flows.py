...
from flowgmm.flow_ssl.distributions_flow import NormalizingFlow, SSLGaussMixturePlanarFlow
...
def CP_set_coverage(logprob_cal, Y_cal, logprob_test, Y_test, p_value=0.05):
    logprob_cal, logprob_test = F.softmax(logprob_cal, dim=1).detach().numpy(), F.softmax(logprob_test, dim=1).detach().numpy()
...
from gde.torchgde import GCNLayer, GDELayer, MLP, GDEFunc, ODEBlock
...
for run_number in range(num_runs):
    if run_number == 1:
        for model_name, model_iter in zip(['MLP','GCN','flow','reversed flow'], [saved_mlp_simple, saved_gcn, saved_flow, saved_reversed_flow]):
            try:
                ...
                print(f"The {model_name} has an error: {e}")
    saved_mlp_simple = None
    saved_gcn = None
    saved_preprocessing = None
    saved_flow = None
    saved_reversed_flow = None
    # saved_linear_transformation = None
    ...
    logprob_cal = logits[cal_mask]
    logprob_test = logits[test_mask]
    ...
    gcn = nn.Sequential(GCNLayer(g=g, in_feats=num_feats, out_feats=16, activation=F.relu, dropout=0.5),
                        ...
                        GCNLayer(g=g, in_feats=16, out_feats=n_classes, activation=None, dropout=0.)
                        )
    ...
    preprocessing = nn.Sequential(GCNLayer(g=g, in_feats=num_feats, out_feats=16, activation=F.relu, dropout=0.5),
                                  GCNLayer(g=g, in_feats=16, out_feats=dim_hidden, activation=None, dropout=0.5))
    # gnn = nn.Sequential(nn.Linear(num_feats, 16),
    #                     nn.ReLU(),
    ...
    gnn = nn.Sequential(GCNLayer(g=g, in_feats=dim_hidden, out_feats=dim_hidden, activation=F.softplus, dropout=0.5),
                        GCNLayer(g=g, in_feats=dim_hidden, out_feats=dim_hidden, activation=F.relu, dropout=0.5),
                        # GCNLayer(g=g, in_feats=dim_hidden, out_feats=dim_hidden, activation=None, dropout=0.5),
                        GCNLayer(g=g, in_feats=dim_hidden, out_feats=dim_hidden, activation=None, dropout=0.)
                        )
    net_dynamics = GDEFunc(gnn) # ODEBlock(odefunc=gdefunc, method='rk4', atol=1e-3, rtol=1e-4, adjoint=False)
    flow = ODEBlock(odefunc=net_dynamics, method='rk4', atol=1e-6, rtol=1e-6, adjoint=True) # FFJORD(net_dynamics, trace_method='hutch', hutch_noise='gaussian', ode_regularization=0)
    ...
    ref_distrib = distrib.MultivariateNormal(torch.zeros(dim_hidden), torch.eye(dim_hidden))
    means = ref_distrib.sample((n_classes, ))

    flows = []
    for class_i in range(n_classes):
        flows.append(NormalizingFlow(dim=dim_hidden, flow_length=1, density=distrib.MultivariateNormal(means[class_i], torch.eye(dim_hidden))))
    ...
    optimizer = torch.optim.Adam(
        list(flow.parameters()) + list(preprocessing.parameters())
        + list(itertools.chain.from_iterable(list(f.parameters()) for f in flows)),# + list(linear_transformation.parameters()), # + [initial_weights],
        lr=lr_init) #, weight_decay=1e-2
    prior = SSLGaussMixturePlanarFlow(flows=flows, means=means)

    for t in range(epochs):
        X_hidden = preprocessing(X)
        # z, sldj, reg_term = flow(X_hidden) #batch_size,
        z, sldj, _ = flow(X_hidden)
        # logits = prior.class_probs(z)
        log_probs, sldj_loss, logits = prior.log_prob(z, labels)
        loss = - log_probs.mean() + sldj_loss - sldj.mean() \
               + 5 * F.cross_entropy(logits[train_mask], Y[train_mask])
        # loss = 5 * F.cross_entropy(logits[train_mask], Y[train_mask]) - loss_fn.prior.log_prob(z, labels).mean() # +  - sldj.mean()
        ...
        if run_number == 0:
            if t % print_freq == 0:
                print(f"iter {t}, log_prob {log_probs.mean()}")
                ...
                # pos = np.dstack((xx, yy))
                for class_i, dist in enumerate(prior.flows):  # prior.gaussians
                    dist = dist.base_density
                    samples = dist.sample((100000,)).detach().numpy()  # failed to find the mean and std of the transformed distribution: ax.scatter(dist.mean[0], dist.mean[1], color=colors[class_i], marker='o', s=50*(dist.stddev[0] + 1)**4, alpha=0.3)
                    counts, ybins, xbins = np.histogram2d(samples[:, 0], samples[:, 1], bins=100)
                    ax.contour(counts, extent=[xbins.min(), xbins.max(), ybins.min(), ybins.max()], levels=[50],
                               colors=colors[class_i], linewidths=.5)
    ...
    prior = SSLGaussMixturePlanarFlow(flows=saved_flows, means=means)
    # saved_linear_transformation.eval()
    X_hidden = saved_flow(saved_preprocessing(X))[0] # saved_preprocessing(X)
    log_probs, loss, logits = prior.log_prob(X_hidden, labels)
    ...
    '''
    # preprocessing = nn.Sequential(GCNLayer(g=g, in_feats=num_feats, out_feats=dim_hidden, activation=F.relu, dropout=0.5))
    #                               # GCNLayer(g=g, in_feats=64, out_feats=dim_hidden, activation=None, dropout=0.))
    # # net_dynamics = GDELayer(g=g, in_feats=dim_hidden, out_feats=dim_hidden, activation=F.softplus, dropout=0.5)
    # gnn = nn.Sequential(GCNLayer(g=g, in_feats=dim_hidden, out_feats=dim_hidden, activation=nn.Softplus(), dropout=0.5),
    #                     GCNLayer(g=g, in_feats=dim_hidden, out_feats=dim_hidden, activation=None, dropout=0.5)
    #                     )
    # gdefunc = GDEFunc(gnn)
    # net_dynamics = GDEFunc(gnn) # ODEBlock(odefunc=gdefunc, method='rk4', atol=1e-3, rtol=1e-4, adjoint=False)
...
labels = ['flow', 'GCN', 'MLP'] #, 'reversed flow', coverages_flow_reverse
fig, ax = plt.subplots()
for i, coverage in enumerate([coverages_flow, coverages_gcn, coverages_mlp_simple]):
