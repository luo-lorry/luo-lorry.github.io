...
class SSLGaussMixture(torch.distributions.Distribution):
    ...
    def __init__(self, means, inv_cov_stds=None, device=None, weights=None):
    ...
    @property
    def prior(self):
        w = torch.lgamma(self.weights.sum(-1)) - torch.lgamma(self.weights.sum(-1) + 1) - torch.lgamma(self.weights).sum(-1)
        w = torch.ones_like(self.weights) * w
        for i in range(w.shape[0]):
            weights_copy = self.weights.clone()
            weights_copy[i] += 1
            w[i] += torch.lgamma(weights_copy).sum(-1)
        return w
    ...
    #     mixture_log_probs = torch.logsumexp(all_log_probs + self.prior_weights, dim=1)
    ...
    #     return torch.cat([g.log_prob(x)[:, None] for g in self.gaussians], dim=1)
