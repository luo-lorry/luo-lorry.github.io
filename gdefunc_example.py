...
for run_number in range(num_runs):
    ...
    gnn = nn.Sequential(GCNLayer(g=g, in_feats=dim_hidden, out_feats=dim_hidden, activation=F.relu, dropout=0.5),
    ...
    flow = ODEBlock(odefunc=net_dynamics, method='rk4', atol=1e-8, rtol=1e-8, adjoint=True) # FFJORD(net_dynamics, trace_method='hutch', hutch_noise='gaussian', ode_regularization=0)
    ...
    for t in range(epochs):
        ...
        # log_probs, loss, logits = prior.log_prob(z, labels)
        loss = - loss_fn.prior.log_prob(z, labels).mean() - sldj.mean() + 10 * F.cross_entropy(logits[train_mask], Y[train_mask])
        ...
        if val_acc > val_acc_flow:
            ...
            # saved_linear_transformation = copy.deepcopy(linear_transformation)
            best_epoch = t
        optimizer.zero_grad()
        ...
        if run_number == 0:
            if t % print_freq == 0:
                ...
                pos = np.dstack((xx, yy))
                for class_i, dist in enumerate(prior.flows.base_density): #prior.gaussians
    ...
    logits = prior.class_probs(X_hidden)
