...
for run_number in range(num_runs):
    ...
    flows = []
    for class_i in range(n_classes):
        flows.append(NormalizingFlow(dim=dim_hidden, flow_length=4, density=distrib.MultivariateNormal(means[class_i], torch.eye(dim_hidden))))
    ...
    for t in range(epochs):
        ...
        log_probs, sldj_loss, logits = prior.log_prob(X_hidden, labels)
        loss = - log_probs.mean() + sldj_loss  \
        ...
        if run_number == 0:
            if t % print_freq == 0:
                ...
                for class_i, dist in enumerate(prior.flows):  # prior.gaussians
                    ...
                    ax.plot(X_hidden[labels == class_i, 0].detach().numpy(), X_hidden[labels == class_i, 1].detach().numpy(),
                            color=colors[class_i], marker='o', linestyle='', markersize=2.8)
                ax.plot(X_hidden[test_mask + val_mask, 0].detach().numpy(),
                        X_hidden[test_mask + val_mask, 1].detach().numpy(), 'ko', markersize=0.4, alpha=0.5)
                ax.set_xlim([-6, 6])
                ax.set_ylim([-6, 6])
    ...
    X_hidden = saved_preprocessing(X) # saved_preprocessing(X)
